...
- [ ] 19. Automatically refresh SAS server content view (SAS extension sidebar) and SAS libraries view after SAS script execution finish
- [ ] 20. Downloading a zip file from the SAS server with the extension resulted in a malformed zip file. Investigate the issue: create tests to validate the issue, test with raw api call, use playwright-cli to test and investigate how SAS Studio web handle it
